...
the idea is simple (sort of): start with a graph. apply a rewriting rule over and over. watch the universe emerge. wolfram thinks this might literally be how spacetime works. i have no idea if he's right, but watching the patterns grow in real time is kind of insane, so i built this.
...
hit run (or just wait a second), and watch the hypergraph evolve step by step. nodes and edges appear according to the rule. the layout is force-directed, so it settles into a shape that reflects the topology — not some arbitrary drawing.
...
3. **steps**: how many times to apply the rule. careful — some rules grow exponentially fast (that's the fun part and also the "oh no" part).
...
the default rule is set to something small so your browser doesn't immediately demand a sacrifice. if you want the full chaos, try `{{x,y},{x,z}} -> {{x,z},{x,w},{y,w},{z,w}}` at step 15.
